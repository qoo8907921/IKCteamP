...
public interface BodyInformationRepositor extends JpaRepository<BodyInformationBean, Integer> {
	...
	Integer findBodyInformationPartofName1(@Param("useraccount")String useraccount);
	
	@Query("SELECT bi.memberheight FROM BodyInformationBean bi WHERE bi.useraccount LIKE CONCAT('',:useraccount,'')")
	Integer findBodyInformationPartofName2(@Param("useraccount")String useraccount);
